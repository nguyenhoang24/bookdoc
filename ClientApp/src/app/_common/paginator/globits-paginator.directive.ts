...
@Directive({
    selector: "[globits-paginator]"
})
export class GlobitsPaginatorDirective {
    private _pageGapTxt = "...";
    ...
    calculatePagesCount(pageSize, totalCount): number {
        // we suppose that if we have 0 items we want 1 empty page
        return totalCount < pageSize ? 1 : Math.ceil(totalCount / pageSize);
    };

    get numOfPages(): number {
        return this.calculatePagesCount(this.matPag.pageSize, this.totalElements)

    }

    get lastPageIndex(): number {
        return this.calculatePagesCount(this.matPag.pageSize, this.totalElements) - 1;
    }
