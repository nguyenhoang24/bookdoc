...
export class RegimenComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.breadcrumbs = [
      {text: "Quản lý danh mục phác đồ", link: null},
    ]
  ...
  pagingUsers() {
    this.isLoading = true;
    this.service.pagingRegimen(this.searchForm.value)
      .subscribe({
        next: (response) => {
  ...
  delete(id: string) {
    this.service.deleteRegimen(id).subscribe(
      {
        next: (response) => {
  ...
  handleOpenPopupFormDrug(id?: string) {
    if (id) {
      this.service.getOne(id).subscribe((response) => {
        if (response) {
          this.dialog.open(RegimenCreateEditComponent, {  height: '250px', width: '550px', data: response })
            .afterClosed().subscribe(this.onClosePopup);
        }
      });
    } else {
      this.dialog.open(RegimenCreateEditComponent, { height: '250px', width: '550px', data: new Drug() })
            .afterClosed().subscribe(this.onClosePopup);
    }
  }
