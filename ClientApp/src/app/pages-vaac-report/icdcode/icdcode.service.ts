...
import { ICDCode } from 'src/app/_models/icdcode.model';
...
export class ICDCodeService {
  ...
  private readonly END_POINT = this.configInitService.apiBaseUrl + '/api/icdCode';
  ...
  saveDrug(icdcode: ICDCode): Observable<ICDCode> {
    var requestHeaders = new HttpHeaders();
    let url = this.END_POINT + "/add"
    return this.http.post<ICDCode>(url, icdcode, { headers: requestHeaders, responseType: "json" });
  }

  updateDrug(icdcode: ICDCode, id: String): Observable<ICDCode> {
    var requestHeaders = new HttpHeaders();
    let url = this.END_POINT + "/update/"+id
    return this.http.put<ICDCode>(url, icdcode, { headers: requestHeaders, responseType: "json" });
  }

  getOne(id: string): Observable<ICDCode>{
    let url = this.END_POINT + "/" + id;
    return this.http.get<ICDCode>(url, { responseType: "json" });
  }

  deleteDrug(id: string): Observable<ICDCode> {
    let url = this.END_POINT + "/delete/" + id
    return this.http.delete<ICDCode>(url, { responseType: "json" });
  }
