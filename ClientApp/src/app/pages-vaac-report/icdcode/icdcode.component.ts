...
import { ICDCodeCreateEditComponent } from './icdcode-create-edit.component';

@Component({
  selector: 'icdcode',
  templateUrl: './icdcode.component.html',
  styleUrls: ['./icdcode.component.scss']
})
export class ICDCodeComponent implements OnInit {
  ...
  columns = [
    ...
    {name: 'Phiên bản', prop: 'version', visible: true},
  ...
  handleOpenPopupFormDrug(id?: string) {
    if (id) {
      this.service.getOne(id).subscribe((response) => {
        if (response) {
          this.dialog.open(ICDCodeCreateEditComponent, {  height: '350px', width: '550px', data: response })
            .afterClosed().subscribe(this.onClosePopup);
        }
      });
    } else {
      this.dialog.open(ICDCodeCreateEditComponent, { height: '350px', width: '550px', data: new Drug() })
            .afterClosed().subscribe(this.onClosePopup);
    }
  }
