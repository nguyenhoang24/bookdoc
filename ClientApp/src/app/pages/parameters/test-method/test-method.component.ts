...
import { TestMethodCreateEditComponent } from './test-method-create-edit/test-method-create-edit.component';
import {TestMethodService} from "./test-method.service";

@Component({
  selector: 'test-method',
  templateUrl: './test-method.component.html',
  styleUrls: ['./test-method.component.scss']
})
export class TestMethodComponent implements OnInit {
  ...
  constructor(
    ...
    private service: TestMethodService
  ) { }
  ...
  reloadTable() {
    this.isLoading = true;
    this.service.pagingTestMethod(this.searchForm.value)
      .subscribe({
        next: (response) => {
  ...
  create() {
    this.dialog.open(TestMethodCreateEditComponent).afterClosed().subscribe(result => {
      this.reloadTable();
    });
  }
  ...
  delete(id: string) {
    this.service.deleteTestMethod(id)
      .subscribe({
        next: () => {
          this.reloadTable();
