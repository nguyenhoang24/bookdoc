...
import { FixedServiceCreateEditComponent } from './fixed-service-create-edit/fixed-service-create-edit.component';
import {FixedServiceService} from "./fixed-service.service";

@Component({
  selector: 'treatment-regimen-stage',
  templateUrl: './fixed-service.component.html',
  styleUrls: ['./fixed-service.component.scss']
})
export class FixedServiceComponent implements OnInit {
  ...
  reloadTable() {
    this.isLoading = true;
    this.service.pagingFixedService(this.searchForm.value)
      .subscribe({
        next: (response) => {
  ...
  create() {
    this.dialog.open(FixedServiceCreateEditComponent).afterClosed().subscribe(result => {
      this.reloadTable();
    });
  }
  ...
  delete(id: string) {
    this.service.deleteFixedService(id)
      .subscribe({
        next: () => {
          this.reloadTable();
