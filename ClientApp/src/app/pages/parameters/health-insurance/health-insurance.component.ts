...
import { HealthInsuranceCreateEditComponent } from './health-insurance-create-edit/health-insurance-create-edit.component';
...
export class HealthInsuranceComponent implements OnInit {
  ...
  constructor(
    ...
    private service: HealthInsuranceService
  ) { }
  ...
  reloadTable() {
    this.isLoading = true;
    this.service.pagingHealthInsurance(this.searchForm.value)
      .subscribe({
        next: (response) => {
  ...
  create() {
    this.dialog.open(HealthInsuranceCreateEditComponent).afterClosed().subscribe(result => {
      this.reloadTable();
    });
  }
  ...
  delete(id: string) {
    this.service.deleteHealthInsurance(id)
      .subscribe({
        next: () => {
          this.reloadTable();
